...
class SGDD:
    ...
    def train(self, verbose=True):
        ...
        self.feat_syn.data.copy_(feat_sub)                          # run time error on this line
        ...
        for it in trange(args.epochs+1):
            ...
            for ol in range(outer_loop):
                ...
                if args.beta > 0:
                    # labels_syn = labels_syn.to(self.device)  # change made
                    # adj_syn = adj_syn.to(self.device)       # change made
                    # onehot_labels_syn = utils.tensor2onehot(labels_syn).to(adj_syn.device)
                    # onehot_labels_syn = utils.tensor2onehot(labels_syn.to(adj_syn.device)).to(adj_syn.device)
                    # onehot_labels_syn = utils.tensor2onehot(labels_syn.to(adj_syn.device)).to(labels_syn.device)


                    # Assuming labels_syn is a tensor
                    # print("label_sy",labels_syn.device )
                    labels_syn = labels_syn.to("cpu")
                    # print("adj_syn",adj_syn.device )
                    onehot_labels_syn = utils.tensor2onehot(labels_syn)
                    onehot_labels_syn=onehot_labels_syn.to(self.device)
                    labels_syn = labels_syn.to(self.device)
                    # print("adj_syn",adj_syn.device )
                    # print("one_hot_lables_syn",onehot_labels_syn.device )



                    loss_reg = args.beta * regularization(adj_syn, onehot_labels_syn)
    ...
    def get_coreset_init(self, features):
        idx_selected_train = np.load(
            f'{self.args.coreset_init_path}/idx_{self.args.dataset}_{self.args.reduction_rate}_{self.args.inittype}_{self.args.seed}.npy')
