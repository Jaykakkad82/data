...
def random_remove_nodes(dataset, p_remove, root, name):
    print('random_remove_nodes called')
...
def random_remove_edges(dataset, p_remove, root, name):
    print('random_remove_edges called')
    # Copy the original data to avoid modifying the original dataset
    data = dataset[0]
    new_data = data.clone()
...
class Pyg2Dpr(Dataset):
    def __init__(self, pyg_data, **kwargs):
        ...
        #deg = degree(pyg_data.edge_index[0], pyg_data.num_nodes)
        #tmp = pyg_data.train_mask
        #train_deg = deg[pyg_data.train_mask]
        #label_train = pyg_data.y.numpy()[pyg_data.train_mask]
        #for m in range(max(label_train)+1):
        #    idx = np.where(label_train==m)
        #    m_degree = train_deg[idx]
        #    avg_m_degree = torch.mean(m_degree)
        #    sum_m_degree = torch.sum(m_degree)
        #    tmp = max(int(sum_m_degree * 0.5), 1)
        #    print(avg_m_degree, sum_m_degree, tmp)
