...
class Dpr2Pyg(InMemoryDataset):
    ...
    def process(self):
        dpr_data = self.dpr_data
        #print('dpr_data.device', dpr_data.device)
        if type(dpr_data.adj) == torch.Tensor:
            adj_selfloop = dpr_data.adj + torch.eye(dpr_data.adj.shape[0]).cuda()
            #adj_selfloop = dpr_data.adj + torch.eye(dpr_data.adj.shape[0]).to(self.device)
            ...
            #print('edge_weight.device', edge_weight.device)
        ...
        try:
            x = torch.FloatTensor(dpr_data.features.cpu()).float().cuda()
            #print('TRY: x.device', x.device)
            #x = torch.FloatTensor(dpr_data.features.cpu()).float().to(self.device)
        except:
            x = torch.FloatTensor(dpr_data.features).float().cuda()
            #print('except: x.device', x.device)
            #x = torch.FloatTensor(dpr_data.features).float()
        try:
            y = torch.LongTensor(dpr_data.labels).cuda()
            #print('TRY: y.device', y.device)
            #y = torch.LongTensor(dpr_data.labels.cpu()).to(self.device)
        except:
            #y = torch.LongTensor(dpr_data.labels)
            y = dpr_data.labels
            #print(y, type(y))
            #print('except: y.device', y.device)
